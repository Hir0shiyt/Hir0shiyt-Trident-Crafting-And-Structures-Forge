...
package net.mcreator.tridentcraftingandstructures.world.features;
...
public class OvergrownTridentFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public OvergrownTridentFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(Blocks.AIR, Blocks.WATER);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("trident_crafting_and_structures", "overgrown_trident"));
		if (template == null)
			return false;
		boolean anyPlaced = false;
		if ((context.random().nextInt(1000000) + 1) <= 350) {
